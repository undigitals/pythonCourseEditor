...
`count()` is useful when you want to count the occurance of an element inside a list.

```python
bunchofnumbers = [1,1,1,10,10]
print(bunchofnumbers.count(1)) # 3
```

The output is 3 as there are 3 ones in `bunchofnumbers` list.

Do not get confuse in `len()` method, we give list as an argument of function `len()`. But in the case of `count()`, we accessing it using `.` (dot).
